...
v11: эмбеддинги через Qwen3-Embedding на vsellm.ru
"""
...
# ═══════════════════════════════════════════════════════════════
# ЭМБЕДДИНГИ — Qwen через vsellm.ru
# ═══════════════════════════════════════════════════════════════
class QwenEmbeddings(Embeddings):
    """Эмбеддинги через /v1/embeddings на vsellm.ru."""
...
Для каждого шага выводи СТРОГО в таком формате — сначала блок карточки, потом текст:

|||STEP_CARD|||
{"step": N, "total": M, "process_name": "название процесса на русском", "title": "название шага", "role": "читаемое название роли на русском", "text": "подробное описание шага из контекста дословно"}
|||END_CARD|||

Правила заполнения JSON:
- `step` и `total` — числа (не строки)
- `process_name` — название процесса из контекста (например "Регистрация нового отчёта")
- `title` — название шага из контекста
- `role` — читаемое имя роли на русском (например "Владелец запроса", "Аналитик данных"), НЕ role_id
- `text` — полное описание шага. Каждое действие с новой строки через \n. Используй формат: "1. Действие\n2. Следующее действие\nПримечание: ..."
- Никакого текста между |||STEP_CARD||| и |||END_CARD||| кроме самого JSON
- После |||END_CARD||| можно добавить короткий комментарий если нужен
...
def parse_step_card(content):
    ...
    Возвращает (card_dict, clean_text) или (None, content).
    """
    import re, json
...
def find_step_meta(docs, step_num):
    """Ищет метаданные шага в RAG-документах по номеру шага."""
    if not docs or not step_num:
        return {}
    for doc in docs:
        if doc.metadata.get("step_number") == step_num:
            return doc.metadata
    return {}


def render_step_card_html(card, docs=None):
    """Рендерит красивую карточку детального шага."""
    step  = card.get("step", "")
    ...
    # Метаданные из FAISS если есть
    meta         = find_step_meta(docs, step)
    location     = meta.get("location", "")
    duration     = meta.get("duration", "")
    process_name = meta.get("process_name", card.get("process_name", ""))

    # Бейджи
    badges = ""
    if role:
        badges += f'<span class="sc-badge sc-badge-role">👤 {role}</span>'
    if location:
        badges += f'<span class="sc-badge sc-badge-loc">📍 {location}</span>'
    if duration:
        badges += f'<span class="sc-badge sc-badge-dur">⏱ {duration}</span>'

    # Строка счётчика + название процесса рядом
    process_html = (
        f' <span class="sc-process">· {process_name}</span>'
        if process_name else ""
    )

    # Рендер текста — каждая строка как отдельный блок
    lines = [l.strip() for l in text.split("\n") if l.strip()]
    text_html = ""
    for line in lines:
        lo = line.lower()
        if lo.startswith("результат") or lo.startswith("итог"):
            text_html += f'<div class="sc-line sc-result">✅ {line}</div>'
        elif lo.startswith("примечание") or lo.startswith("важно") or lo.startswith("внимание"):
            text_html += f'<div class="sc-line sc-note">💡 {line}</div>'
        elif line[0].isdigit() and len(line) > 2 and line[1] in ".):":
            text_html += f'<div class="sc-line sc-action">{line}</div>'
        else:
            text_html += f'<div class="sc-line">{line}</div>'

    html = f"""
<div class="sc-card">
  <div class="sc-header">
    <div class="sc-top">
      <span class="sc-counter">ШАГ {step} ИЗ {total}</span>{process_html}
    </div>
    <div class="sc-title">{title}</div>
  </div>
  <div class="sc-badges">{badges}</div>
  <div class="sc-body">{text_html}</div>
</div>"""
