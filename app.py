...
SYSTEM_PROMPT = """Ты — Глосси, ИИ-ассистент по работе с отчетами в Бизнес-Глоссарии (БГ) Банка.

## Твоя роль и контекст
Ты действуешь как опытный аналитик данных ДБД, который досконально знает процессы работы с отчетами в информационной системе Бизнес-Глоссарий (БГ) Банка.

БГ (Бизнес-глоссарий) — это корпоративная система управления отчетным ландшафтом Банка. В ней регистрируются, актуализируются и согласуются все отчеты подразделений. Основные виды запросов в БГ:
- **Регистрация нового отчета** — внесение нового отчета в реестр отчетов Бизнес-Глоссария
- **Актуализация отчета** — обновление данных существующего отчета
- **Исключение отчета** — исключение отчета из реестра отчетов Бизнес-Глоссария
- **Смена владельца отчета** — передача ответственности другому ССП (подразделению)
- **Автоматизация отчета** — разработка автоматизированного отчета (нового или на основе существующего ручного отчета)

Каждый вид запроса — это процесс из нескольких шагов с участием разных ролей: Владелец запроса (ВЗ), Аналитик данных ДБД, Владелец отчета (ВО), Представители согласования от ССП.

Ты помогаешь сотрудникам Банка разобраться в этих процессах: объясняешь шаги, поля для заполнения, правила согласования и навигацию по системе.
...
Контекст состоит из чанков. Каждый чанк — это шаг или фрагмент процесса работы с БГ.

Поля каждого чанка:
- `topic` — тема чанка: название вида запроса + название шага (например: «Регистрация нового отчета — Заполнение карточки запроса»)
- `process_name` — название вида запроса в БГ (например: «Регистрация нового отчета», «Актуализация отчета», «Исключение отчета», «Смена владельца»)
- `step_number` — порядковый номер шага внутри процесса (0 = обзор всего процесса, 1,2,3... = конкретные шаги)
- `part_title` — название раздела/части процесса к которому относится шаг
- `keywords` — ключевые слова для поиска
- `source` — имя файла-источника

Типы действий внутри шага (поле `type` у листьев):
- `action` — что нужно сделать (кнопка, действие в интерфейсе)
- `field` — поле формы которое нужно заполнить (с инструкцией и примером)
- `tab` — вкладка в карточке запроса/отчета, содержит поля
- `note` — важное примечание или предупреждение
- `result` — результат выполнения шага
- `branch` — ветвление: если условие → то действие
- `shared_reference` — ссылка на общий блок (например атрибутный состав)

Другие поля у листьев:
- 'variant_for_process' - уточняет к какому типу запроса применимо данное поле. Если указано all - значит ко всем запросам. Если указан конкретный process_id, то выводи информацию только если пользователь спросил про данный вид запроса 

Поля у processess: 
'applies_to' - указывает к каким типам отчетов применимо
'flow summary' - список шагов для процесса, ориентируйся на них выводя краткую информацию о шагах, можешь на них ориентироваться и при выводе подробных шагов для понимания какие шаги применимы для конкретного вида запроса, так как набор шагов зависит от вида запроса

Поля у "steps":
'deadline': максимальная длительность выполнения шага
'role' - кто выполняет шаг
'location' - в какой среде выполняется шаг
'duration' - длительность исполнения шага
'bg_step_name' - название шага в системе Бизнес-глоссарий: нужно чтобы человек мог сопоставить как шаг называется в системе (указывается в правом верхнем углу карточки)
'in process' - в каких процессах данный шаг участвует (нужно для построения шагов процесса), если стоит 'all' - то для вех процессов применимо

## ОЦЕНКА ВОПРОСА ПЕРЕД ОТВЕТОМ
Перед каждым ответом мысленно оцени (не пиши это вслух):
1. Что именно спрашивает пользователь?
2. К какому виду запроса (process_name) относится вопрос?
3. К какому типу отчета относится запрос?
4. Какие чанки из контекста наиболее релевантны?
5. Достаточно ли информации для ответа?

## КВАЛИФИКАЦИЯ ЗАПРОСА
В начале каждого ответа ОБЯЗАТЕЛЬНО определи тематику одной строкой:
`🏷️ Тема: [название темы]`
Темы: Регистрация отчета | Актуализация | Исключение отчета | Смена владельца | Автоматизация | Атрибутный состав | Навигация и поиск | Реестр отчетов | Общие вопросы | Другое

## УТОЧНЕНИЕ ТИПА ЗАПРОСА
Если пользователь спрашивает КАК СДЕЛАТЬ что-то в БГ — проверь, указан ли тип запроса.

Слова-триггеры (НЕ переспрашивай) для определения вида запроса:
- «новый отчет», «зарегистрировать», «создать отчет», «внести в БГ» → Регистрация нового отчета
- «актуализировать», «обновить отчет», «изменить отчет» → Актуализация отчета
- «автоматизировать», «автоматизация», «BIQ», «бизнес-инициатива» → Автоматизация отчета
- «исключить», «удалить отчет» → Удаление отчета
- «сменить владельца», «поменять ССП», «изменить департамент», «сменить подразделение» → Смена владельца отчета

ОБЯЗАТЕЛЬНО уточняй вид запроса если он НЕ определен по триггерам — не выдавай шаги пока не знаешь вид запроса. Спроси: «Уточните, пожалуйста, о каком виде запроса идет речь?» и перечисли варианты списком.
После получения ответа — выдавай шаги ТОЛЬКО для этого вида запроса.
НЕ смешивай шаги разных видов запросов в одном ответе.

## РАБОТА С ПОШАГОВЫМИ ИНСТРУКЦИЯМИ

### Два режима ответа
Когда пользователь просит объяснить процесс — спроси:

«Как вам удобнее?
📋 **Кратко** — все шаги одним списком (напиши 1)
🔍 **Детально** — разбираем каждый шаг подробно, по одному» (напили 2)

**📋 Кратко:** Перечисли все шаги нумерованным списком. На каждый — 1 строка с названием. Только из контекста, опирайся на flow summaty.

**🔍 Детально:** Выдавай ОДИН шаг за раз. В конце добавь: `[NEXT_STEP_AVAILABLE]`.
На «следующий»/«дальше»/«далее»/«продолжай» — выдай следующий шаг.
Формат СТРОГО:

|||STEP_CARD|||
{"step": N, "total": M, "process_id": "process_id из контекста", "process_name": "название вида запроса на русском"}
|||END_CARD|||

### Порядок шагов — КРИТИЧЕСКИ ВАЖНО
- Выводи шаги СТРОГО В ПОРЯДКЕ step_number из контекста
- Нумерация шагов может быть дробной: 1, 1.1, 1.2, 2.1, 2.2, 4, 5... — это нормально, не придумывай промежуточные шаги
- НЕ пропускай шаги которые есть в контексте
- НЕ придумывай шаги которых НЕТ в контексте — если после шага 2.2 идёт шаг 4, значит шагов 3 не существует
- НЕ объединяй шаги, если они разделены в контексте
- Чанк с step_number=0 — это обзор процесса, используй для общего ответа
- `total` в карточке — это общее количество шагов из flow_summary для данного процесса

Правила JSON:
- `step` и `total` — числа (не строки)
- `process_id` — точный id (например "proc_registration")
- `process_name` — название вида запроса (например "Регистрация нового отчета")
- Никакого текста между маркерами кроме JSON
- После |||END_CARD||| НЕ добавляй описание — оно покажется автоматически
- Формат |||STEP_CARD||| используй ТОЛЬКО в режиме 🔍 Детально и ТОЛЬКО для одного шага
- Если пользователь просит "дальше" после последнего шага — напиши: «Это был последний шаг процесса "[название процесса]". Могу ли я помочь вам с каким-то конкретным шагом или другим видом запроса?»

## Что ты умеешь
1. Отвечать на вопросы по работе с отчетами в БГ
2. Объяснять типы запросов и типы отчетов
3. Проводить по процессу: кратко или детально по одному шагу
4. Объяснять заполнение полей, карточки запроса, атрибутного состава
5. Подсказать, где найти реестр отчетов (БГ или платформа УОЛ)

## Что ты НЕ умеешь
- Давать информацию по конкретным отчетам подразделений
...
Скажи: «У меня нет информации по этому вопросу в базе знаний. Обратитесь к команде Управления отчетным ландшафтом или Аналитики данных — контакты в разделе «Контакты».»
Не придумывай ответ.
"""
...
def normalize_yo(text: str) -> str:
    """Нормализует е→е для надежного распознавания."""
    return text.replace('е', 'е').replace('Е', 'Е')

# Маппинг триггеров → (process_name, process_id)
PROCESS_TRIGGERS = {
    "регистрац":          ("Регистрация нового отчета", "proc_registration"),
    "новый отчет":        ("Регистрация нового отчета", "proc_registration"),
    "внести в бг":        ("Регистрация нового отчета", "proc_registration"),
    "внести в глоссарий": ("Регистрация нового отчета", "proc_registration"),
    "актуализ":           ("Актуализация отчета",       "proc_update"),
    "обновить отчет":     ("Актуализация отчета",       "proc_update"),
    "изменить отчет":     ("Актуализация отчета",       "proc_update"),
    "исключ":             ("Удаление отчета",           "proc_delete"),
    "удалить отчет":      ("Удаление отчета",           "proc_delete"),
    "смен":               ("Смена владельца отчета",    "proc_rochange"),
    "поменять ссп":       ("Смена владельца отчета",    "proc_rochange"),
    "изменить департамент":("Смена владельца отчета",   "proc_rochange"),
    "сменить подразделение":("Смена владельца отчета",  "proc_rochange"),
    "владелец":           ("Смена владельца отчета",    "proc_rochange"),
    "автоматиз":          ("Автоматизация отчета",      "proc_automation"),
    "biq":                ("Автоматизация отчета",      "proc_automation"),
    "бизнес-инициатив":   ("Автоматизация отчета",      "proc_automation"),
    "атрибут":            ("Атрибутный состав",         ""),
    "ас ":                ("Атрибутный состав",         ""),
}

def detect_process_type(text: str):
    """Возвращает (process_name, process_id) или (None, None)."""
    text_norm = normalize_yo(text).lower()
    for trigger, (pname, pid) in PROCESS_TRIGGERS.items():
        if normalize_yo(trigger).lower() in text_norm:
            return pname, pid
    return None, None

def rerank_docs(question: str, docs: list, scores: list, api_key: str, top_n: int = 5) -> tuple:
    """Реранкинг чанков через второй вызов Qwen.
    Возвращает топ-N наиболее релевантных документов.
    """
    if len(docs) <= top_n:
        return docs, scores

    # Готовим список чанков для реранкера
    chunks_text = ""
    for i, doc in enumerate(docs, 1):
        meta = doc.metadata
        label = f"[{i}]"
        if meta.get("process_name"):
            label += f" {meta['process_name']}"
        if meta.get("step_number", 0) > 0:
            label += f", шаг {meta['step_number']}"
        snippet = doc.page_content[:300].replace("\n", " ")
        chunks_text += f"{label}: {snippet}\n\n"

    rerank_prompt = (
        f"Вопрос пользователя: «{question}»\n\n"
        f"Ниже {len(docs)} фрагментов из базы знаний. "
        f"Выбери номера {top_n} наиболее релевантных для ответа на вопрос.\n"
        f"Ответь ТОЛЬКО списком номеров через запятую, например: 2, 5, 7, 1, 9\n\n"
        f"{chunks_text}"
    )

    try:
        client = openai.OpenAI(api_key=api_key, base_url=API_BASE_URL, timeout=30)
        ...
        # Парсим номера
        import re
        nums = [int(x.strip()) for x in re.findall(r'\d+', raw) if 1 <= int(x.strip()) <= len(docs)]
        # Убираем дубли, сохраняем порядок
        seen = set()
        indices = [n-1 for n in nums if n-1 not in seen and not seen.add(n-1)]
        # Добиваем до top_n если парсер вернул меньше
        for i in range(len(docs)):
            if len(indices) >= top_n:
                break
            if i not in indices:
                indices.append(i)
        reranked_docs   = [docs[i]   for i in indices[:top_n]]
        reranked_scores = [scores[i] for i in indices[:top_n]]
        return reranked_docs, reranked_scores
    except Exception:
        # Если реранкер упал — возвращаем оригинал
...
def rag_answer(question: str, vectorstore, api_key: str, k: int = TOP_K):
    ...
    if process_id:
        ...
        all_docs = vectorstore.similarity_search_with_score(process_name, k=100)
    ...
    for i, doc in enumerate(docs_sorted, 1):
        ...
        if meta.get("step_number") and str(meta["step_number"]) not in ("0", "0.0", "-1"):
            header += f", шаг {meta['step_number']}"
        if meta.get("part_title"):
            header += f", раздел: {meta['part_title']}"
        if meta.get("role"):
            header += f", роль: {meta['role']}"
        header += "]"
...
def apply_variant(leaf: dict, pid: str) -> dict:
    """Применяет вариант листа для конкретного процесса, включая поля внутри вкладок."""
    variants = leaf.get("variants", {})
    override = variants.get(pid, {}) if variants else {}
    result = {**leaf, **override}
    if "variants" in result:
        del result["variants"]
    # Рекурсивно применяем варианты для полей внутри вкладки
    if leaf.get("type") == "tab" and leaf.get("fields"):
        result["fields"] = [apply_variant(f, pid) for f in leaf["fields"]]
    return result
...
def build_card_css() -> str:
    ...
    return """
<style>
body{margin:0;padding:0;overflow:hidden;line-height:normal}
.sc-card{background:white;border:1px solid #d1fae5;border-left:4px solid #10b981;
  border-radius:0 12px 12px 0;padding:14px 16px;margin:0 0 0 0;font-family:sans-serif}
...
<div class="sc-card">
  ...
  <div class="sc-body">{leaves_html}</div>
    ...
    n_fields_in_tabs_simple = sum(1 for l in leaves if l.get("type") == "tab" for f in l.get("fields",[]) if not f.get("values"))
    n_fields_in_tabs_values = sum(1 for l in leaves if l.get("type") == "tab" for f in l.get("fields",[]) if f.get("values"))
    n_fields_standalone = sum(1 for l in leaves if l.get("type") == "field")
    base   = 110 if not leaves else 120
    height = max(base, base + n_actions * 46 + n_tabs * 38 + n_fields_in_tabs_simple * 80 + n_fields_in_tabs_values * 170 + n_fields_standalone * 70)
...
def render_assistant_message(content, log_id, avg_score=0.0,
    ...
    if card:
        ...
        # Кнопки навигации всегда показываем при карточке шага
        step_num = card.get("step", 0)
    ...
    # Старые кнопки пошагового режима (фолбэк)
    if next_step and not card:
        col1, col2, col3 = st.columns([1, 1, 2])
        with col1:
            if st.button("▶ Следующий шаг", key=f"next_fb_{log_id}", use_container_width=True):
                st.session_state.pending_question = "следующий шаг"
                st.rerun()
        with col2:
            if st.button("↩ К началу", key=f"restart_fb_{log_id}", use_container_width=True):
                st.session_state.pending_question = "начни сначала"
                st.rerun()
...
Помогает разобраться в процессах управления отчетным ландшафтом,
отвечает на вопросы и проводит по шагам регистрации отчетов.
    """)
    ...
    with col1:
        ...
        st.markdown("""
1. Отвечать на общие вопросы по отчетам и БГ
2. Рассказать про типы запросов и отчетов
...
6. Отвечать на вопросы по управлению отчетным ландшафтом
        """)
 
    with col2:
        st.markdown("### ❌ Что пока не умею")
        st.markdown("""
- Давать информацию по конкретным отчетам
...
- Создавать карточки запросов
    ...
    st.markdown("---")
    st.info(
        "Не нашли ответ? Обратитесь к команде **Управления отчетным ландшафтом** "
...
def main():
    ...
    # chat_input — закреплен внизу страницы
