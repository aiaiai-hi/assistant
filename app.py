...
Ты действуешь как опытный аналитик данных ДБД, который досконально знает процессы работы с отчётами в информационной системе Бизнес-Глоссарий (БГ) Банка.
...
- **Регистрация нового отчёта** — внесение нового отчета в реестр отчетов Бизнес-Глоссария
- **Актуализация отчёта** — обновление данных существующего отчета
- **Исключение отчёта** — исключение отчета из реестра отчетов Бизнес-Глоссария
- **Смена владельца отчёта** — передача ответственности другому ССП (подразделению)
- **Автоматизация отчёта** — разработка автоматизированного отчета (нового или на основе существующего ручного отчета)
...
Поля каждого чанка:
- `topic` — тема чанка: название вида запроса + название шага (например: «Регистрация нового отчёта — Заполнение карточки запроса»)
- `process_name` — название вида запроса в БГ (например: «Регистрация нового отчёта», «Актуализация отчёта», «Исключение отчёта», «Смена владельца»)
...
- `tab` — вкладка в карточке запроса/отчёта, содержит поля
...
- «исключить», «удалить отчёт» → Исключение отчета из реестра отчетов
...
- `process_name` — название вида запроса (например "Регистрация нового отчёта")
...
def rag_answer(question: str, vectorstore, api_key: str, k: int = TOP_K):
    ...
    docs_list, scores_list = rerank_docs(question, docs_list, scores_list, api_key, top_n=5)
